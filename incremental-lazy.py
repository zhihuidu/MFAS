...
def iterative_solver_indicator(graph,edge_flag):
    ...
    #total_constraints += num_violations
    start_time=0
    end_time=0
    original_time=time.time()
    start_time=time.time()
    cur_constraint = 0
    print(f"half of edges is {num_edges}")
    for u, v in list(graph.edges())[:1]:
        model.addConstr(p[u]+ 1 <= p[v] + M * (1 - x[(u, v)]), f"order_{u}_{v}")

    # Callback function to add constraints in batches
    def lazy_callback(model, where):
        nonlocal iter, lower_bound, upper_bound, removed_weight, total_constraints, num_violations, start_time, end_time, original_time, cur_constraint,num_edges

        if where == GRB.Callback.MIPSOL:
            end_time=time.time()
            #removed_weight = sum(graph[u][v]['weight']  for u, v in graph.edges()  if x[(u, v)].X < 0.5 )
            removed_weight = sum(graph[u][v]['weight'] for u, v in graph.edges() if model.cbGetSolution(x[(u, v)]) < 0.5)
            upper_bound = total_weight - removed_weight
            lower_bound = removed_weight
            iter +=1
            print(f"------  Here ---------------")
            persec = int (num_violations/ (end_time-start_time))
            print(f"{iter} iteration, this step added {cur_constraint}")
            print(f"totally added {total_constraints} constraints, this step took {end_time-start_time}s")
            if total_constraints != 0:
                print(f"current speed is {persec} violations/s, average speed is {int(total_constraints/(end_time-original_time))}, estimated #day {(65.48287 *(end_time-original_time)/total_constraints):.1f}")
            ...
            cur_constraint=0
            start_time=time.time()
            print(f"totally has {len(graph.edges())} edges")
            begin=total_constraints
            total_constraints=0
            for u, v in list(graph.edges())[num_edges:]:
                total_constraints+=1
                p_u = model.cbGetSolution(p[u])
                p_v = model.cbGetSolution(p[v])
                x_uv = model.cbGetSolution(x[(u, v)])
                #print(f"for edge <{u},{v}>")
                if (p_u == p_v) or (p_u > p_v and x_uv > 0.5 ):
                #if (p[u].X==p[v].X) or (p[u].X>p[v].X and x[(u,v)].X==1):
                    model.cbLazy(p[u]+ 1 <= p[v] + M * (1 - x[(u, v)]))
                    cur_constraint +=1
    ...
    if model.status == GRB.Status.OPTIMAL:
            print(f"**************************")
            print(f"the optimization result is {model.objVal}")
            removed_weight = sum(graph[u][v]['weight']  for u, v in graph.edges()  if x[(u, v)].X < 0.5 )
            removededge=[]
            for u, v in graph.edges() :
                if x[(u, v)].X < 0.5:
